    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50 px-4 lg:px-6 h-16 flex items-center bg-gray-950 shadow-lg transition-all duration-300 ease-in-out">
            ...
            <nav className="ml-auto flex items-center gap-4 sm:gap-6">
                <Link className={`text-sm font-medium hidden lg:block hover:text-emerald-500 transition-colors ${currentPathname === '/' ? 'text-emerald-500' : ''}`} href="/">
                    Home
                </Link>
                <Link className={`text-sm font-medium hidden lg:block hover:text-emerald-500 transition-colors ${currentPathname === '/aboutus' ? 'text-emerald-500' : ''}`} href="/aboutus">
                    About Us
                </Link>
                {session && <Link className="text-sm font-medium hidden lg:block hover:text-emerald-500 transition-colors" href="/profile">
                    Create
                </Link>}
                {!session ? (<Link className="text-sm font-medium hover:text-emerald-500 transition-colors" href="#" onClick={props.signInWithGoogle}>
                    Sign In
                </Link>) : (<div className="">
                    <DropdownMenu >
